...
git clone https://github.com/wizwizdev/wizwizxui-timebot.git /var/www/html/dd-timebot
sudo chown -R www-data:www-data /var/www/html/dd-timebot/
sudo chmod -R 755 /var/www/html/dd-timebot/
...
(crontab -l ; echo "* * * * * curl https://${DOMAIN_NAME}/dd-timebot/settings/gift2all.php >/dev/null 2>&1") | sort - | uniq - | crontab -
(crontab -l ; echo "*/3 * * * * curl https://${DOMAIN_NAME}/dd-timebot/settings/tronChecker.php >/dev/null 2>&1") | sort - | uniq - | crontab -
...
wait
    ...
    if [[ $(mysql -u root -p$ROOT_PASSWORD -e "SHOW DATABASES LIKE 'wizwiz2'") ]]; then
        ...
        file_path="/var/www/html/dd-timebot/baseInfo.php"
        ...
        # print file
        echo -e "<?php" >> /var/www/html/dd-timebot/baseInfo.php
        echo -e "error_reporting(0);" >> /var/www/html/dd-timebot/baseInfo.php
        echo -e "${ASAS}botToken = '${YOUR_BOT_TOKEN}';" >> /var/www/html/dd-timebot/baseInfo.php
        echo -e "${ASAS}dbUserName = '${dbuser}';" >> /var/www/html/dd-timebot/baseInfo.php
        echo -e "${ASAS}dbPassword = '${dbpass}';" >> /var/www/html/dd-timebot/baseInfo.php
        echo -e "${ASAS}dbName = '${dbname}';" >> /var/www/html/dd-timebot/baseInfo.php
        echo -e "${ASAS}botUrl = 'https://${YOUR_DOMAIN}/dd-timebot/';" >> /var/www/html/dd-timebot/baseInfo.php
        echo -e "${ASAS}admin = ${YOUR_CHAT_ID};" >> /var/www/html/dd-timebot/baseInfo.php
        echo -e "?>" >> /var/www/html/dd-timebot/baseInfo.php

        sleep 1

        curl -F "url=https://${YOUR_DOMAIN}/dd-timebot/bot.php" "https://api.telegram.org/bot${YOUR_BOT_TOKEN}/setWebhook"
        ...
        url="https://${YOUR_DOMAIN}/dd-timebot/createDB.php"
        curl $url
        
        sleep 1
        
        sudo rm -r /var/www/html/dd-timebot/webpanel
        sudo rm -r /var/www/html/dd-timebot/install
        sudo rm /var/www/html/dd-timebot/createDB.php
	rm /var/www/html/dd-timebot/updateShareConfig.php
	rm /var/www/html/dd-timebot/README.md
	rm /var/www/html/dd-timebot/README-fa.md
	rm /var/www/html/dd-timebot/LICENSE
	rm /var/www/html/dd-timebot/update.sh
	rm /var/www/html/dd-timebot/wizwiz.sh
	rm /var/www/html/dd-timebot/tempCookie.txt
	rm /var/www/html/dd-timebot/settings/messagewizwiz.json
