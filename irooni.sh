...
if [ ! -d "/root/conirooni" ]; then

    sudo mkdir /root/conirooni
    
    sleep 1
    
    touch /root/conirooni/dbrootwizwiz.txt
    ...
    echo "${ASAS}user = 'root';" >> /root/conirooni/dbrootwizwiz.txt
    echo "${ASAS}pass = '${randomdbpasstxt}';" >> /root/conirooni/dbrootwizwiz.txt
    #echo "${ASAS}paths = '$RANDOM_CODE';" >> /root/conirooni/dbrootwizwiz.txt
    
    sleep 1

    passs=$(cat /root/conirooni/dbrootwizwiz.txt | grep '$pass' | cut -d"'" -f2)
    userrr=$(cat /root/conirooni/dbrootwizwiz.txt | grep '$user' | cut -d"'" -f2)
...
# update cron
PATHS=$(cat /root/conirooni/dbrootwizwiz.txt | grep '$path' | cut -d"'" -f2)
...
ROOT_PASSWORD=$(cat /root/conirooni/dbrootwizwiz.txt | grep '$pass' | cut -d"'" -f2)
ROOT_USER="root"
