...
update_function() {
  if [ -d "amir-marzban-admin" ]; then
    cp amir-marzban-admin/config.json .
    
    rm -rf amir-marzban-admin
    
    git clone https://github.com/Real-kia/kia-marzban-admin
    
    mv config.json amir-marzban-admin/
    
    sudo systemctl restart amirmarzbanbot.service
    
    echo "Update completed successfully."
    sleep 3
  else
    echo "Error: Could not find the 'amir-marzban-admin' folder."
    sleep 3
  fi
}
...
install_function() {
    ...
    cat > amir-marzban-admin/config.json << EOF
{
    "bot_token": "$bot_token",
...
EOF
    ...
    cat << EOF | sudo tee /etc/systemd/system/amirmarzbanbot.service
[Unit]
Description=amir Marzban Bot Service
After=network.target

[Service]
User=$(whoami)
WorkingDirectory=$bot_dir/amir-marzban-admin
ExecStart=/usr/bin/python3 $bot_dir/amir-marzban-admin/$bot_script
Restart=always
RestartSec=10
...
EOF
    ...
    sudo systemctl enable amirmarzbanbot.service

    # Start the bot service
    echo "Starting the bot service..."
    sudo systemctl start amirmarzbanbot.service
...
uninstall_function() {
    ...
    sudo systemctl stop amirmarzbanbot.service

    # Disable the service to prevent it from starting on boot
    sudo systemctl disable amirmarzbanbot.service

    # Remove the service file
    sudo rm /etc/systemd/system/amirmarzbanbot.service
...
status_function() {
    ...
    sudo systemctl status amirmarzbanbot.service --no-pager
    sleep 10
}
